...
import (
	...
	"github.com/koKurahuang/capital-construction/log/modNameHook"
	...
	"path"
	"runtime"
	"github.com/koKurahuang/capital-construction/log/funcNameHook"
)
...
func updateLogInstance(ins *logrus.Logger, c *logConfig) {
	...
	consoleFlg := config.GetBool("log.enableconsole")
	if !consoleFlg {
		ins.Out = nullIO
	}
...
func getLogConfig() *logConfig {
	//config.InitConfig()
	return &logConfig{
		formatter:  config.GetString("log.format"),
		isDump:     config.GetBool("log.dumpFile.enable"),
		dumpPath:   config.GetPath("log.dumpFile.path"),
		dumpFormat: config.GetString("log.dumpFile.format"),
		timestamp:  config.GetString("log.timestamp"),
		moduleList: config.GetStringMap("log.level"),
		colored:    config.GetBool("log.enablecolors"),
	}
}
